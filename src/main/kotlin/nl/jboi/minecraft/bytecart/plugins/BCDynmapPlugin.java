...
public final class BCDynmapPlugin implements Listener {
    ...
    private static final String name = ByteCart.plugin.getConfig().getString("dynmap.layer", "ByteCart");
    ...
    public BCDynmapPlugin() {
        ...
        defaulticon = loadIcon(markerapi, "bytecart_station", "station", "icons/bc2.png");

        if (defaulticon == null) {
            throw new MissingResourceException("Station icon could not be loaded", MarkerAPI.class.getName(), "station");
        }

        erroricon = loadIcon(markerapi, "bytecart_error", "error", "icons/error.png");

        if (erroricon == null) {
            throw new MissingResourceException("Error icon could not be loaded", MarkerAPI.class.getName(), "error");
        }

        updatericon = loadIcon(markerapi, "bytecart_updater", "updater", "icons/updater.png");
     ...
     * @param loc        the location to draw at
    ...
    public static void removeObsoleteMarkers() {
        if (markerset != null)
            Bukkit.getScheduler().runTask(ByteCart.plugin, new searchObsoleteMarkers());
    }
