        ...
        if (this.getVehicle() != null) {
            ...
            if (this.getInput(0).getAmount() > 0) {
                if (this.wasTrain(this.getLocation()))
                    ByteCart.plugin.getIsTrainManager().getMap().reset(this.getLocation());
                ...
                ByteCart.plugin.getServer().getScheduler().scheduleSyncDelayedTask(ByteCart.plugin, new Runnable() {
                            @Override
                            public void run() {
...
// if the cart is stopped, start it
                if (this.getVehicle().getVelocity().equals(new Vector(0, 0, 0))) {
                    ...
                    this.getVehicle().setVelocity((new Vector(this.getCardinal().getModX(), this.getCardinal().getModY(), this.getCardinal().getModZ())).multiply(ByteCart.plugin.getConfig().getDouble("BC7001.startvelocity")));
                }
            }
            ...
            // if the wire is off
                ...
                if (!this.wasTrain(getLocation())) {
                    ...
                    ByteCart.plugin.getIsTrainManager().getMap().remove(getBlock().getRelative(getCardinal().getOppositeFace(), 2).getLocation());
                } else
                    ByteCart.plugin.getIsTrainManager().getMap().reset(this.getLocation());
