...
public final class MinecartChunkManager {
    ...
    public static int clean() {
        ...
        for (World world : Bukkit.getWorlds()) {
            ...
            if (tickets.containsKey(ByteCart.plugin)) {
                for (Chunk chunk : tickets.get(ByteCart.plugin)) {
    ...
    public static boolean checkAndUnregister(Chunk chunk) {
        if (chunk.getPluginChunkTickets().contains(ByteCart.plugin) || !chunk.isLoaded())
            return false;
        ...
        chunk.removePluginChunkTicket(ByteCart.plugin);
        return true;
    }

    public static void register(Chunk chunk) {
        if (chunk.getPluginChunkTickets().contains(ByteCart.plugin))
            return;

        if (ByteCart.debug)
            ByteCart.log.info(String.format("ByteCart: ChunkManager : add @ x%d z%d", chunk.getX(), chunk.getZ()));

        chunk.addPluginChunkTicket(ByteCart.plugin);
    }
}
