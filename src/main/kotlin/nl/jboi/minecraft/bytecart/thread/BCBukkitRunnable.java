...
final class BCBukkitRunnable<K> {
    ...
    BukkitTask renewTaskLater(long duration, Object... objects) {
        ...
        synchronized (map) {
            if (!Expirable.getThreadMap().containsKey(Key)) {
                ...
                if (old.isSync())
                    task = runnable.runTaskLater(ByteCart.plugin, duration);
                else
                    task = runnable.runTaskLaterAsynchronously(ByteCart.plugin, duration);
                old.cancel();
            }
    ...
    private BukkitTask runTaskLater(Object... objects) {
        BukkitRunnable runnable = new Expire(Expirable, Key, objects);
        BukkitTask task = runnable.runTaskLater(ByteCart.plugin, Expirable.getDuration());
        return task;
    }
    ...
    private BukkitTask runTaskLaterAsynchronously(Object... objects) {
        BukkitRunnable runnable = new Expire(Expirable, Key, objects);
        BukkitTask task = runnable.runTaskLaterAsynchronously(ByteCart.plugin, Expirable.getDuration());
        return task;
    }
