...
public abstract class AbstractIC implements IC {
    ...
    public static boolean checkEligibility(Block b) {
        ...
        if (ByteCart.plugin.getConfig().getBoolean("FixBroken18", false)) {
    ...
    public final int getTriggertax() {
        return ByteCart.plugin.getConfig().getInt("usetax." + this.getName());
    }

    @Override
    public final int getBuildtax() {
        return ByteCart.plugin.getConfig().getInt("buildtax." + this.getName());
    }
